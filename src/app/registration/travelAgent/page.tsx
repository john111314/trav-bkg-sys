...
export default function TA_Registration() {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const licenseNoInput = document.getElementById("license_no") as HTMLInputElement;
        ...
        if (!firstNameInput || !lastNameInput || !emailInput || !phoneInput ||
            !addressInput || !genderSelect || !licenseNoInput || !passwordInput) {
        ...
        let licenseNo = licenseNoInput.value;
        ...
        const licenseNoPattern = /^[A-Z][0-9]{9}$/;
        if (!licenseNoPattern.test(licenseNo)) {
            document.getElementById("passwordError")!.textContent = "License number must be a 10 digit string, containing a capital letter as the first character, and numbers for the remaining 9 digits.";
            isValid = false;
        }
       ...
       if (isValid) {
        try {
            const response = await fetch('/api/registration/travelAgent', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    license_no: licenseNoInput.value.trim(),
            ...
            document.getElementById("message")!.textContent = error.message || 'An unexpected error occurred. Please try again.';
   ...
   return (
     <div className="container">
         ...
         <form id="registrationForm" onSubmit={handleSubmit}>
             ...
             <div className="error" id="licenseError"></div>
